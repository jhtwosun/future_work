"""
Experiment E7: QwQ-32B (Qwen reasoning model) on MATH-500.

QwQ-32B is Qwen's 32B reasoning model, more recent than R1-Distill.
Long-CoT generation expected — we use 100 questions × N=4 SC like
Pilot G2 to fit in budget.
"""
...
GREEDY_OUT = OUTDIR / "E7_qwq32b_greedy.jsonl"
SC_OUT = OUTDIR / "E7_qwq32b_sc.jsonl"
SUM = OUTDIR / "E7_summary.json"
...
def main():
    ...
    print(f"Loading {MODEL} (32B reasoning)...", flush=True)
    t_load = time.time()
    llm = LLM(
        model=MODEL,
        dtype="bfloat16",
    ...
    with GREEDY_OUT.open("w") as f:
        for i, (q, gold, out) in enumerate(zip(questions, golds, outs)):
            ...
            n_steps_all.append(len(boundaries)); n_tokens_all.append(len(tok_ids))
            f.write(json.dumps({
    ...
    summary["greedy"] = {"accuracy": n_correct / len(prompts),
                          ...
                          "median_steps": float(np.median(n_steps_all)),
                          "mean_tokens": float(np.mean(n_tokens_all))}
    print(f"QwQ greedy: {n_correct}/{len(prompts)} = {n_correct/len(prompts):.3f}  steps mean={np.mean(n_steps_all):.1f}  tok mean={np.mean(n_tokens_all):.0f}", flush=True)
    ...
    print(f"QwQ SC@{N_SAMPLES}: maj={n_maj/len(prompts):.3f}  any={n_any/len(prompts):.3f}", flush=True)
