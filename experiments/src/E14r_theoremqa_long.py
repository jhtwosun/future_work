"""
Rerun E14 with MAX_TOKENS=2560 (was 1536). Original ~13% truncated.
"""
...
MAX_TOKENS = int(os.environ.get("MAXTOK", "2560"))
TEMP_SC = 0.7
SEED = 0

OUTDIR = Path("/home/nvidia/future/experiments/results")
GREEDY_OUT = OUTDIR / "E14r_theoremqa_greedy.jsonl"
SC_OUT = OUTDIR / "E14r_theoremqa_sc.jsonl"
SUM = OUTDIR / "E14r_summary.json"
...
def thm_equal(pred, gold, gold_type):
    ...
    if gold_type == "bool":
        ...
        # Stricter bool check
        if "true" in gold_str:
            return "true" in pred_str and "false" not in pred_str
        if "false" in gold_str:
            return "false" in pred_str and "true" not in pred_str
        return False
    if gold_type == "option":
...
def main():
    ...
    keep = [i for i, r in enumerate(ds) if not r.get("Picture")]
    questions = [ds[i]["Question"] for i in keep]
    golds = [ds[i]["Answer"] for i in keep]
    types = [ds[i]["Answer_type"] for i in keep]
    print(f"  {len(questions)} text-only problems", flush=True)
    ...
    summary = {"model": MODEL + " (no-think)", "n": len(prompts), "dataset": "TheoremQA",
                "by_answer_type": dict(Counter(types)), "max_tokens": MAX_TOKENS}
    ...
    type_correct = Counter(); type_counter = Counter(types)
