"""
Experiment E1: Full MATH-500 (all 500 problems) with greedy + SC@8.

Pilot 7 already gave us 500 greedy traces. Pilot 9 gave us 200 SC@8 traces.
This run extends SC@8 to the full 500 to enable proper bootstrap CIs and
uniformly compare LP, PRM (Pilot A also has 500), and SC scores.

Output:
  results/E1_math500_greedy_traces.jsonl  (500 traces with token logprobs)
  results/E1_math500_sc8_traces.jsonl     (500 traces × 8 samples)
...
GREEDY_OUT = OUTDIR / "E1_math500_greedy_traces.jsonl"
SC_OUT = OUTDIR / "E1_math500_sc8_traces.jsonl"
SUM = OUTDIR / "E1_summary.json"
...
def main():
    ...
    print(f"SC@{N_SAMPLES}: maj={n_maj}/{len(prompts)} ({n_maj/len(prompts):.3f}) any={n_any/len(prompts):.3f}", flush=True)
