"""
Experiment E8: Microsoft Phi-4 (14B) on MATH-500.
Cross-family validation — does the CoT-CP story hold beyond Qwen?
"""
...
GREEDY_OUT = OUTDIR / "E8_phi4_greedy.jsonl"
SC_OUT = OUTDIR / "E8_phi4_sc.jsonl"
SUM = OUTDIR / "E8_summary.json"
...
def main():
    ...
    summary["greedy"] = {"accuracy": n_correct / len(prompts), "parse_failures": n_parse_fail, "wallclock_sec": dur_g}
    print(f"Phi-4 greedy: {n_correct/len(prompts):.3f}", flush=True)
    ...
    print(f"Phi-4 SC@{N_SAMPLES}: maj={n_maj/len(prompts):.3f}", flush=True)
