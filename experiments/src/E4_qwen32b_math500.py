"""
Experiment E4: Qwen2.5-32B-Instruct on MATH-500 (greedy + SC@4).

Scaling test from research_plan §1.3 #6: does the trajectory-level
CP-on-SC story hold at 32B?

We use SC@4 instead of SC@8 to fit in time budget at 32B size.
"""
...
GREEDY_OUT = OUTDIR / "E4_qwen32b_math500_greedy.jsonl"
SC_OUT = OUTDIR / "E4_qwen32b_math500_sc.jsonl"
SUM = OUTDIR / "E4_summary.json"
...
def main():
    ...
    print(f"Loading {MODEL} (32B - ~64GB GPU)...", flush=True)
    t_load = time.time()
    llm = LLM(
        model=MODEL,
        dtype="bfloat16",
    ...
    print(f"32B greedy: {n_correct}/{len(prompts)} = {n_correct/len(prompts):.3f}", flush=True)
    ...
    print(f"32B SC@{N_SAMPLES}: maj={n_maj}/{len(prompts)} = {n_maj/len(prompts):.3f}", flush=True)
