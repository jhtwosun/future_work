"""
Experiment E18: DeepSeek-R1-Distill-Llama-70B on MATH-500 (TP=2).

Largest model in this study. ~140GB bf16 — requires tensor parallelism
across both H100 NVL GPUs (190GB total). Long-CoT reasoning.

Greedy on full 500. SC@4 on 100 problems (long-CoT is very expensive at 70B).
"""
...
N_SC = int(os.environ.get("N_SC_PROBLEMS", "100"))
MAX_TOKENS = int(os.environ.get("MAXTOK", "8192"))
TEMP_SC = 0.7
SEED = 0

OUTDIR = Path("/home/nvidia/future/experiments/results")
GREEDY_OUT = OUTDIR / "E18_r1_distill_llama_70b_greedy.jsonl"
SC_OUT = OUTDIR / "E18_r1_distill_llama_70b_sc.jsonl"
SUM = OUTDIR / "E18_summary.json"
...
def main():
    ...
    print(f"Loading {MODEL} (70B, TP=2, ~140GB bf16)...", flush=True)
    t_load = time.time()
    llm = LLM(
        model=MODEL,
        dtype="bfloat16",
